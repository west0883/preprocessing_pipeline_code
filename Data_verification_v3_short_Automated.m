...
function [ ] = Data_verification_v3_short_Automated(day)

% Navigate to your stacks folder
day=uigetdir; 
...
for s=1:length(day_dir)
    ...
    % Draw ROIs: 1 in th brain; 1 on dental cement
    if first==1

        % Load in first frame to draw ROI
        ...
        imshow(image,[low high]); %%  change depending on lighting levels  %%%
        title({'Click the vertices to define an ROI in the brain (avoid large blood vessels).','To close the ROI, right-click on 1st point and create mask for each.'},'fontsize',18);
        hold on

        % Draw ROI over brain
        roi_1_mask=roipoly;   
        roi_1_border=cell2mat(bwboundaries(roi_1_mask));
        plot(roi_1_border(:,2),roi_1_border(:,1),'r','LineWidth',1);

        title({'Click the vertices to define an ROI over dental cement (bright part).','To close the ROI, right-click on 1st point and create mask for each.'},'fontsize',18);
        
        % Draw ROI over dental cement
        roi_2_mask=roipoly;  
...
for b=1:size(bCheck_all,3)
    if size(bCheck_all,3)<7 && size(bCheck_all,3)<7
